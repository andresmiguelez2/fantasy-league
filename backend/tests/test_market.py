...
class AssignBidsTests(unittest.TestCase):
    ...
    def test_assigns_footballer_and_debits_bidder_budget(self):
        ...
        bid_data = [
            (1, 10, 42, 1000, None)
        ]  # bid_id=1, footballer_id=10, bidder_id=42, amount=1000, seller_id=None
        ...
        update_footballer_call = any(
            "UPDATE footballer" in s and "owner_id" in s for s in sql_strings
        )
        update_budget_call = any(
            "UPDATE player" in s and "budget" in s for s in sql_strings
        )
        ...
        args = budget_call.args[
            1
        ]  # second positional arg to execute() is the params tuple
    ...
    def test_no_budget_update_when_bidder_is_none(self):
        ...
        self.assertEqual(
            len(budget_calls), 0, "Should not update budget when bidder_id is None"
        )
    ...
    def test_highest_bid_wins_and_gets_debited(self):
        ...
        budget_call = next(
            (
                ...
                if "UPDATE player" in str(c) and "budget" in str(c)
            ),
            None,
        )
        ...
        self.assertEqual(
            args,
            (1500, 42, 7),
            "Should debit the winning bidder (42) with the winning amount (1500)",
        )
    ...
    def test_assigns_footballer_and_credits_seller_budget(self):
        ...
        bid_data = [
            (1, 10, 42, 1000, 9)
        ]  # bid_id=1, footballer_id=10, bidder_id=42, amount=1000, seller_id=9
        cursor = self._make_cursor(bid_data)
        ...
        credit_call = next(
            (
                c
                for c in execute_calls
                if "UPDATE player" in str(c) and "budget = budget + %s" in str(c)
            ),
            None,
        )
        ...
        self.assertEqual(
            args, (1000, 9, 7), "Should credit seller (9) with sold amount (1000)"
        )
