...
class FootballerRouterTests(unittest.TestCase):
    ...
    def test_get_all_footballers_applies_filters_and_returns_filter_options(self, mock_pg_connect):
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = (1,)
        mock_cursor.fetchall.side_effect = [
            [(1, "Player One", "Team A", 1000000, "Owner", 5.5, 100, "fw", "available")],
        ...
        self.assertEqual(response["meta"]["filter_options"], {
            "teams": ["Team A", "Team B"],
            "positions": ["df", "fw"],
