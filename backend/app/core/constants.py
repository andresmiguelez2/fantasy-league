...
HANDLE_DANGLING_FIXTURES_INTERVAL = (
    50  # dangling features will be handled every number of iterations
)
UPDATE_FIXTURES_INTERVAL = (
    200  # dangling features will be handled every number of iterations
)
...
FANTASY_PLAYER_MARKET_URL = (
    "https://www.futbolfantasy.com/analytics/laliga-fantasy/mercado/detalle/"
)
...
INITIAL_SQUAD_TOTAL_VALUE_LIMIT = (
    100_000_000  # max combined squad value on entry (100 M)
)
INITIAL_SQUAD_PLAYER_VALUE_LIMIT = (
    30_000_000  # max value per individual footballer (30 M)
)
INITIAL_SQUAD_TOTAL_VALUE_TOLERANCE = (
    0.10  # squad value must be within ±10% of the total limit
)
...
RELEASE_CLAUSE_DAYS = (
    14  # number of days until a footballer can be released again after being bought
)
BID_EXPIRATION_DAYS = 3  # number of days until a bid expires
MAX_DEBT_AS_VALUE_UNIT = (
    0.2  # maximum debt allowed as a percentage of the player's budget
)
PLACE_ON_MARKET_WITH_RELEASE_CLAUSE = (
    False  # True if a footballer can be placed on the market with a release clause
)
