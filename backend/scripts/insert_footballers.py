...
if __name__ == "__main__":
    ...
    player_data_df = pd.DataFrame(
        get_all_players(soup), columns=["name", "full_name", "displayable_name"]
    )
    ...
    for _, row in tqdm(player_data_df.iterrows()):
        try:
            ...
            footballer = Footballer(
                obtain_data=True, name=row["name"], full_name=row["full_name"]
            )
            footballer.name = (
                row["displayable_name"] if row["displayable_name"] else row["name"]
            )
            ...
            if footballer.data["market_details"]:
                ...
                cursor.execute(
                    ...
                    (
                        ...
                        footballer.team,
                        footballer.data["market_details"][-1]["value"],
                        footballer.data["total_points"],
                        footballer.data["average_points"],
                        FOOTBALLER_POSITIONS[footballer.data["position"]],
                        footballer.data["availability"],
                    ),
                )
