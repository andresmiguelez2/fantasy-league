...
from app.classes.fixture import Fixture, get_current_fixture
...
class GetCurrentFixtureTests(unittest.TestCase):
    ...
    def test_returns_first_non_dangling_fixture(self, mock_pg_connect):
        ...
        mock_conn, mock_cursor = self._mock_connection([("fixture-1", start_ts, time_open)])
    ...
    @patch("app.classes.fixture.pg_connect")
    def test_returns_none_when_only_dangling(self, mock_pg_connect):
        start_ts = datetime(2025, 1, 1, 12, 0, 0)
        time_open = timedelta(days=6)

        mock_conn, _ = self._mock_connection([("fixture-1", start_ts, time_open)])
        mock_pg_connect.return_value = mock_conn
    ...
    @patch("app.classes.fixture.pg_connect")
    def test_skips_dangling_then_returns_next(self, mock_pg_connect):
        start_ts_a = datetime(2025, 1, 1, 12, 0, 0)
        start_ts_b = datetime(2025, 1, 2, 12, 0, 0)

        open_fixtures = [
            ("dangling-1", start_ts_a, timedelta(days=7)),
            ("fixture-2", start_ts_b, timedelta(days=1)),
        ]
